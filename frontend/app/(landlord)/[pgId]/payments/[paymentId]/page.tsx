    ...
    return (
      <>
        ...
        <div className="mt-6 text-center py-12 bg-white rounded-lg border border-slate-200">
          ...
          <Button onClick={() => router.push(`/${pgId}/payments`)} className="mt-4">
            Back to Payments
          </Button>
        </div>
      </>
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => router.push(`/${pgId}/payments`)}>
              Back
            </Button>
            {payment.status !== 'PAID' && (
              <Button onClick={() => router.push(`/${pgId}/payments/${paymentId}/record`)}>
                Record Payment
              </Button>
            )}
          </div>
      ...
      <div className="mt-6 space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="mt-1 flex items-center text-slate-900">
                  ...
                  <button
                    onClick={() => router.push(`/${pgId}/tenants/${payment.lease.tenant.id}`)}
              ...
              <div>
                ...
                <div className="mt-1 flex items-center text-slate-900">
                  ...
                  <button
                    onClick={() => router.push(`/${pgId}/properties/${payment.lease.unit.property.id}`)}
                    className="hover:underline"
                  >
                    {payment.lease.unit.property.propertyName} - {payment.lease.unit.unitName}
                  </button>
                </div>
              </div>
              <div>
              ...
              {payment.datePaid && (
                <div>
                  <label className="text-sm font-medium text-slate-600">Date Paid</label>
                  <div className="mt-1 flex items-center text-slate-900">
                    <Calendar className="w-4 h-4 mr-2 text-slate-400" />
            ...
            {toFiniteNumber(payment.amountDue) > toFiniteNumber(payment.amountPaid) && (
              <div className="pt-4 border-t border-slate-200">
                ...
                <p className="mt-1 text-red-600 font-semibold text-lg">
                  {formatPeso(toFiniteNumber(payment.amountDue) - toFiniteNumber(payment.amountPaid))}
                </p>
              </div>
            )}
          </CardContent>
