...
function landlordLeafLabel(parts: string[], index: number) {
  ...
  if (prev === 'properties' && parts[index + 1] === 'units') return 'Property Details';
...
function buildLandlordCrumbs(pathname: string, isAdmin: boolean): Crumb[] {
  ...
  for (let i = 1; i < parts.length; i += 1) {
    ...
    const isEntityId =
      ['properties', 'tenants', 'leases', 'payments', 'units'].includes(prevSegment) &&
      segment !== 'new' &&
      segment !== 'record';
...
export function AppBreadcrumb({ isAdmin }: { isAdmin: boolean }) {
  ...
  const groupName = propertyGroup?.group?.name ?? propertyGroup?.group?.groupName;
  const groupCode = formatPgCode(propertyGroup?.group?.pgNumber ?? null);

  const crumbs = useMemo(() => {
    if (pathname.startsWith('/dashboard') || pathname === '/landlords' || pathname === '/subscriptions' || pathname === '/settings') {
        ...
        return (
          <div key={`${crumb.label}-${idx}`} className="flex items-center gap-2">
            ...
            {crumb.href && !isLast ? (
              ...
              <span className={cn(isLast && 'font-semibold text-slate-700')}>{crumb.label}</span>
            )}
