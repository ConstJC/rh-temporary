...
function getActiveNavHref(
  ...
  for (const item of navItems) {
    ...
    if (isPathMatch(pathname, item.href)) {
      score = Math.max(score, item.href.length);
    }
...
const adminNav: readonly NavItem[] = [
  ...
  {
    href: "/dashboard/property-groups",
  ...
  return [
    {
      ...
      menuCode: "LANDLORD_DASHBOARD",
    },
    {
      href: `/${pgId}/properties`,
      label: "Properties",
      icon: Building2,
      menuCode: "LANDLORD_PROPERTIES",
    },
    {
      href: `/${pgId}/tenants`,
      label: "Tenants",
      icon: Users,
      menuCode: "LANDLORD_TENANTS",
    },
    {
      href: `/${pgId}/leases`,
      label: "Tenant Leases",
      icon: FileClock,
      menuCode: "LANDLORD_LEASES",
    },
    {
      href: `/${pgId}/payments`,
      label: "Payments",
      icon: CreditCard,
      menuCode: "LANDLORD_PAYMENTS",
    },
    {
      href: `/${pgId}/addons`,
      label: "Add-ons",
      icon: Cable,
      menuCode: "LANDLORD_ADDONS",
    },
    {
      href: `/${pgId}/utilities`,
      label: "Utilities",
      icon: Handshake,
      menuCode: "LANDLORD_UTILITIES",
    },
    {
      href: `/${pgId}/reports`,
      label: "Reports",
      icon: BarChart3,
      menuCode: "LANDLORD_REPORTS",
    },
    {
      href: `/${pgId}/subscription`,
      label: "Subscription",
      icon: BadgeDollarSign,
      menuCode: "LANDLORD_SUBSCRIPTION",
    },
    {
      href: `/${pgId}/settings`,
      label: "Settings",
      icon: Settings,
      menuCode: "LANDLORD_SETTINGS",
    },
  ];
}
...
export function Sidebar({
  ...
  const [settingsOpen, setSettingsOpen] = useState(
    isPathMatch(pathname, "/dashboard/settings"),
  );
  const isSettingsRoute = isPathMatch(pathname, "/dashboard/settings");
  const isSettingsExpanded = open && (settingsOpen || isSettingsRoute);
  const subscriptionQuery = useSubscription(pgId ?? "");
  const allowedMenus = new Set(subscriptionQuery.data?.plan.access.menus ?? []);
  const hasAccessPayload = subscriptionQuery.isSuccess;

  const navItems = isAdmin
    ? adminNav
    : getLandlordNav(pgId).filter(
        (item) =>
          !item.menuCode ||
          !hasAccessPayload ||
          allowedMenus.has(item.menuCode),
      );
    ...
    <>
      ...
      >
        <div className="flex h-full w-full flex-col pt-6">
          <div
            className={cn(
              "flex items-center gap-3 px-5 pb-6",
          ...
          <nav className="flex-1 space-y-1 px-3 py-2">
            {navItems.map((item) => {
              if (item.children) {
                const parentActive = activeNavHref === item.href;
