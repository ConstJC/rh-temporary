  ...
  return [
    {
      ...
      accessorKey: "action",
      cell: ({ row }) => (
        <StatusBadge status={actionKey(row.original.action)} />
      ),
    },
    {
      header: "Table",
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-slate-700">{row.original.recordId}</span>
      ),
    },
    {
      header: "IP Address",
      ...
      cell: ({ row }) => (
        <span className="text-slate-600">{row.original.ipAddress ?? "—"}</span>
      ),
    },
    {
      header: "Timestamp",
