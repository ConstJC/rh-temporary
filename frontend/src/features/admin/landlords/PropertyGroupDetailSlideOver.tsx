...
export function PropertyGroupDetailSlideOver({
  ...
  return (
    ...
    >
      {!group ? (
        <p className="text-sm text-slate-500">Loading property group details...</p>
      ) : (
        <div className="space-y-4">
          <div>
            ...
            {isEditMode ? (
              <div className="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">Currency Code</label>
                  <input
                    value={currencyCode}
          ...
          <div className="space-y-3 border-t border-slate-200 pt-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Properties & Units</p>
            {group.properties.length === 0 ? (
              <p className="text-sm text-slate-500">No properties found for this group.</p>
            ) : (
              group.properties.map((property) => (
                <div key={property.id} className="rounded-lg border border-slate-200 p-3">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <p className="text-sm font-semibold text-slate-900">{property.propertyName}</p>
                      <p className="text-xs text-slate-500">{property.propertyType.replaceAll('_', ' ')}</p>
                      <p className="mt-1 text-xs text-slate-600">{property.address}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Units</p>
                      <p className="text-sm font-semibold text-slate-900">{property.unitCount}</p>
                    </div>
                  </div>

                  <div className="mt-2 flex flex-wrap gap-2">
                    {Object.entries(property.unitStatusCounts).map(([status, count]) => (
                      <span
                        key={status}
                  ...
                  <div className="mt-3 space-y-1">
                    {property.units.length === 0 ? (
                      <p className="text-xs text-slate-500">No units in this property.</p>
                    ) : (
                      property.units.map((unit) => (
                        ...
                        >
                          <div>
                            <p className="text-sm font-medium text-slate-900">{unit.unitName}</p>
                            <p className="text-xs text-slate-500">{unit.unitType.replaceAll('_', ' ')}</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <StatusBadge status={unit.status} />
                            <span className="text-xs font-semibold text-slate-700">
                              {formatCurrency(unit.monthlyRent, group.currencyCode)}
                            </span>
                          </div>
                        </div>
                      ))
