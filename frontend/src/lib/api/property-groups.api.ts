...
export async function listPropertyGroups(): Promise<PropertyGroupSummary[]> {
  const { data } = await apiClient.get<{ data: PropertyGroupSummary[] }>(
    API_ENDPOINTS.PROPERTY_GROUPS,
  );
  return (data.data ?? []).map((g) => ({
    ...
    pgCode:
      g.pgCode ??
      (typeof g.pgNumber === "number"
        ? `PG-${String(g.pgNumber).padStart(3, "0")}`
        : undefined),
  }));
}
...
export async function createPropertyGroup(
  payload: CreatePropertyGroupDto,
): Promise<PropertyGroupSummary> {
  const { data } = await apiClient.post<{ data: PropertyGroupSummary }>(
    API_ENDPOINTS.PROPERTY_GROUPS,
    payload,
  );
...
export async function updatePropertyGroup(
  id: string,
  payload: UpdatePropertyGroupDto,
): Promise<PropertyGroupSummary> {
  const { data } = await apiClient.patch<{ data: PropertyGroupSummary }>(
    `${API_ENDPOINTS.PROPERTY_GROUPS}/${id}`,
    payload,
  );
  const group = data.data;
  return {
